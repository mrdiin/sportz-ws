...
async function fetchMatches(limit = 100) {
  const response = await fetch(`${API_URL}/api/v1/matches?limit=${limit}`);
  if (!response.ok) {
...
async function insertCommentary(matchId, entry) {
  ...
  const response = await fetch(`${API_URL}/api/v1/matches/${matchId}/commentary`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
function normalizeCricketFeed(entries, match) {
  ...
  for (const key of inningsKeys) {
    ...
    const other = inningsEntries.filter(
      (entry) => entry.team && entry.team !== 'neutral' && entry.team !== primaryTeam && entry.team !== secondaryTeam
    );
